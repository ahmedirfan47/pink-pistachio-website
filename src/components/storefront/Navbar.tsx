  ...
  return (
    <header
      className={cn(
        ...
        scrolled ? 'bg-cream/90 shadow-sm backdrop-blur-md' : 'bg-cream/60 backdrop-blur-sm'
      )}
    >
      <div className="container-px mx-auto flex h-20 max-w-7xl items-center justify-between">
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <Link href="/cart" className="relative rounded-full p-2 transition-colors hover:bg-pink-100" aria-label="Cart">
              ...
              <div className="absolute right-0 top-12 w-56 rounded-2xl border border-pink-100 bg-white p-2 shadow-lg">
                {session ? (
                  <>
                    ...
                    <Link href="/account" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm hover:bg-pink-50">
                      <User className="h-4 w-4" /> My Account
                    </Link>
                    <Link href="/account/orders" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm hover:bg-pink-50">
                      <ClipboardList className="h-4 w-4" /> My Orders
                    </Link>
                    {session.user?.role === 'ADMIN' && (
                      <Link href="/admin/dashboard" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium text-pink-600 hover:bg-pink-50">
                        Admin Dashboard
                      </Link>
                    )}
                    <button
                  ...
                  <div className="flex flex-col gap-1 p-1">
                    <Link href="/login" className="rounded-xl px-3 py-2 text-sm hover:bg-pink-50">
                      Login
                    </Link>
                    <Link href="/register" className="rounded-xl bg-pink-50 px-3 py-2 text-sm font-medium text-pink-700 hover:bg-pink-100">
                      Create Account
                    </Link>
                  </div>
                )}
        ...
        <div className="border-t border-pink-100 bg-cream px-4 pb-6 pt-2 lg:hidden">
          <nav className="flex flex-col gap-1">
            {NAV_LINKS.map((link) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === link.href ? 'bg-pink-50 text-pink-600' : 'text-charcoal'
                )}
              >
                {link.label}
              </Link>
            ...
            <div className="mt-2 flex flex-col gap-2 border-t border-pink-100 pt-3">
              {session ? (
                <>
                  <Link href="/account" className="rounded-xl px-3 py-3 text-sm">My Account</Link>
                  <Link href="/account/orders" className="rounded-xl px-3 py-3 text-sm">My Orders</Link>
                  {session.user?.role === 'ADMIN' && (
                    <Link href="/admin/dashboard" className="rounded-xl px-3 py-3 text-sm font-medium text-pink-600">Admin Dashboard</Link>
                  )}
                  <button onClick={() => { signOut({ callbackUrl: '/' }); }} className="rounded-xl px-3 py-3 text-left text-sm text-red-600">Logout</button>
                </>
              ) : (
                <>
                  <Link href="/login" className="rounded-xl px-3 py-3 text-sm">Login</Link>
                  <Link href="/register" className="rounded-xl bg-pink-50 px-3 py-3 text-sm font-medium text-pink-700">Create Account</Link>
                </>
              )}
              <Link href="/menu" className="btn-primary mt-2 w-full">Order Now</Link>
            </div>
          </nav>
        </div>
      )}
