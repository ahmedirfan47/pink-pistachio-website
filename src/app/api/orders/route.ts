...
const placeOrderSchema = checkoutSchema.and(
  z.object({ items: z.array(orderItemSchema).min(1, 'Cart is empty') })
);
...
export async function POST(req: Request) {
  try {
    ...
    const deliveryFee = data.deliveryType === 'pickup' ? 0 : subtotal - discount >= freeDeliveryMin ? 0 : deliveryFeeBase;
    ...
    if (err?.issues) {
      return NextResponse.json({ error: err.issues[0]?.message || 'Invalid order data' }, { status: 400 });
    }
