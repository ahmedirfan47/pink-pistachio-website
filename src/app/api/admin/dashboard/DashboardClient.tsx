  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <StatCard label="Today's Revenue" value={formatPrice(stats.todayRevenue)} icon={TrendingUp}
          trend={stats.todayOrders + ' orders today'} trendUp color="pink" />
        <StatCard label="Pending Orders"  value={String(stats.pendingOrders)}     icon={Clock}
          trend={stats.pendingOrders > 0 ? 'Needs attention' : 'All clear'} trendUp={stats.pendingOrders === 0} color="gold" />
      </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {stats.recentOrders.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="py-6 text-center text-charcoal-600">No orders yet.</td>
                  </tr>
