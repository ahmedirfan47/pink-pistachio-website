...
interface Category { id: string; name: string; slug: string; description?: string; image?: string; position: number; isActive: boolean }
...
export default function CategoriesClient() {
  ...
  const openEdit   = (c: Category) => {
    setEditing(c);
    setForm({ name:c.name, slug:c.slug, description:c.description??'', image:c.image??'', position:String(c.position), isActive:c.isActive });
    ...
    <div>
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {loading ? <tr><td colSpan={5} className="py-14 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin text-pink-500" /></td></tr>
            : categories.map(c => (
              <tr key={c.id} className="border-b border-pink-50 hover:bg-pink-50/20">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    {c.image && <div className="relative h-8 w-8 shrink-0 overflow-hidden rounded-lg"><Image src={c.image} alt={c.name} fill className="object-cover" /></div>}
                ...
                <td className="px-4 py-3">
                  <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${c.isActive ? 'bg-pistachio-100 text-pistachio-700' : 'bg-red-100 text-red-700'}`}>{c.isActive ? 'Active' : 'Hidden'}</span>
                </td>
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <button onClick={() => openEdit(c)} className="rounded-lg p-1.5 text-charcoal-600 hover:bg-pink-100 hover:text-pink-600"><Pencil className="h-4 w-4" /></button>
                    <button onClick={() => del(c.id)} className="rounded-lg p-1.5 text-charcoal-600 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-charcoal/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-hover">
            ...
            <div className="space-y-4">
              <div><label className="label-field">Name</label><input value={form.name} onChange={e => setForm((f:any)=>({...f,name:e.target.value}))} className="input-field" /></div>
              <div><label className="label-field">Slug</label><input value={form.slug} onChange={e => setForm((f:any)=>({...f,slug:e.target.value}))} className="input-field" /></div>
              <div><label className="label-field">Description</label><textarea value={form.description} onChange={e => setForm((f:any)=>({...f,description:e.target.value}))} rows={2} className="input-field" /></div>
              ...
              <div>
                ...
                {form.image && <div className="relative mb-2 h-24 w-24 overflow-hidden rounded-2xl"><Image src={form.image} alt="" fill className="object-cover" /></div>}
              ...
              <label className="flex cursor-pointer items-center gap-2 text-sm">
                <input type="checkbox" checked={form.isActive} onChange={e => setForm((f:any)=>({...f,isActive:e.target.checked}))} className="h-4 w-4 accent-pink-600" /> Active
              </label>
