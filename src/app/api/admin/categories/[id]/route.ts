...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const slugConflict = await db.category.findFirst({ where: { slug: data.slug, NOT: { id: params.id } } });
    if (slugConflict) return NextResponse.json({ error: 'A category with this slug already exists.' }, { status: 409 });
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (productCount > 0) {
      return NextResponse.json({ error: `Cannot delete — ${productCount} product(s) use this category. Move or delete them first.` }, { status: 400 });
    }
