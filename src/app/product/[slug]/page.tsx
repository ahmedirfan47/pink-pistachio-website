  ...
  return (
    <div className="container-px mx-auto max-w-7xl py-10">
      ...
      <nav className="mb-6 flex items-center gap-2 text-sm text-charcoal-600">
        ...
        <ChevronRight className="h-3.5 w-3.5" />
        <Link href={`/menu?category=${product.category.slug}`} className="hover:text-pink-600">{product.category.name}</Link>
        <ChevronRight className="h-3.5 w-3.5" />
      ...
      <div className="grid gap-10 lg:grid-cols-2">
        <div className="relative aspect-square overflow-hidden rounded-3xl bg-pink-50 shadow-sm">
          {product.images[0] ? (
            ...
            <div className="flex h-full items-center justify-center text-pink-300">No image available</div>
          )}
          {!product.isAvailable && (
            <div className="absolute inset-0 flex items-center justify-center bg-white/70">
              <span className="rounded-full bg-charcoal px-5 py-2 text-sm font-semibold text-white">Sold Out</span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col">
          ...
          <h1 className="font-display text-3xl font-bold text-charcoal sm:text-4xl">{product.name}</h1>

          <div className="mt-4 flex flex-wrap gap-2">
            {product.tags.map((tag) => (
              <span key={tag} className="rounded-full bg-pistachio-100 px-3 py-1 text-xs font-semibold capitalize text-pistachio-600">
                {tag.replace('-', ' ')}
              </span>
            ))}
          </div>
          ...
          <div className="mt-6 flex items-baseline gap-3">
            <span className="font-display text-3xl font-bold text-pink-600">{formatPrice(product.price)}</span>
            {product.compareAtPrice && product.compareAtPrice > product.price && (
              <span className="text-base text-charcoal-600/60 line-through">{formatPrice(product.compareAtPrice)}</span>
            )}
          </div>
          ...
          {product.stock > 0 && product.stock <= 5 && (
            <p className="mt-3 text-sm font-medium text-amber-600">Only {product.stock} left — order soon!</p>
          )}
          ...
          <div className="mt-10 rounded-2xl bg-pink-50 p-5 text-sm text-charcoal-600">
            ...
            <p className="mt-1">Available for pickup from DHA Raya &amp; Gulberg, or delivered across Lahore. Free delivery on orders over Rs. 3,000.</p>
          </div>
        </div>
      </div>
